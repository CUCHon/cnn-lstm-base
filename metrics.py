...
def _imp_generator(predicted_imps, batch_idx, vocab):#根据id生成第一句/imp
    impression_ids = predicted_imps[batch_idx]
    impression_words = []
    for word_id in impression_ids:
        word = vocab.id2word[word_id]
        if word == '<start>' or word == '<pad>' or word == "<unk>": #无视
            continue
        elif word == '<end>':# 停止
            break
        else:
            impression_words.append(word) #添加
    impression = ' '.join(impression_words)#连成句子
    impression = impression.lower()#小写
    return impression


def _fin_generator(predicted_fins, batch_idx, vocab):#生成后面的句子/findings
    ...
    for num_sen in range(finding_ids.shape[0]):
        ...
        if not single_sentence:                        #生成空句子就停
            break
        else:
...
def _gt_imp_generator(para):#调整真实数据impresions的格式
    para = para.split('.')
    gt_imp = ''
    ...
    return gt_imp.lower().replace(', ', ' , ').replace('  , ', ' , ')


def _gt_fin_generator(para):  #调整真实数据findings的格式
    return para.lower().replace(', ', ' , ').replace('. ', ' . ')


def _generate_imp_fin_dict(predicted_imps_lst, predicted_fins_lst, image_ids_lst, args): #生成词典
    pre_imp_dict = {}
    pre_fin_dict = {}
    pre_imp_fin_dict = {}
    gt_imp_dict = {}
...
def generate_text_file(predicted_imps_lst, predicted_fins_lst, image_ids_lst, num_run, args):#输出到txt文件
    gt_imp_dic, pre_imp_dic, gt_fin_dic, pre_fin_dic, gt_imp_fin_dic, pre_imp_fin_dic = \
        _generate_imp_fin_dict(predicted_imps_lst, predicted_fins_lst, image_ids_lst, args)
