...
from Model.load_data import get_loader
from Model.ImageEncoder import ImageEncoder
from Model.SGMDecoder import Sentence_Generative_Model_Decoder
from Model.RPGmodels import  Recurrent_Paragraph_Generative_Decoder
from metrics import compute_metrics, generate_text_file
from Model.logger import create_logger
from Data.build_vocab import JsonReader, Vocabulary
...
def train_net(num_run, logger, args):
    ...
    data_loader = get_loader(args.image_dir, args.json_dir,
                             ...
                             shuffle=True)
    # Models
    image_encoder = ImageEncoder().train().to(device)
    impression_decoder = Sentence_Generative_Model_Decoder(args.embed_size, args.hidden_size,
    ...
    finding_decoder = Recurrent_Paragraph_Generative_Decoder(args.embed_size, args.hidden_size, vocab_size,
...
def test_in_training(image_encoder, impression_decoder, finding_decoder, vocab, args):
    ...
    # Models
    image_encoder_eval = ImageEncoder().eval().to(device)
    impression_decoder_eval = Sentence_Generative_Model_Decoder(args.embed_size, args.hidden_size,
                                                 vocab_size, args.imp_layers_num,
                                                 args.num_global_features, args.num_conv1d_out,
                                                 args.teach_rate, args.max_impression_len).eval().to(device)
    finding_decoder_eval = Recurrent_Paragraph_Generative_Decoder(args.embed_size, args.hidden_size, vocab_size,
                                             args.fin_num_layers, args.sen_enco_num_layers,
...
if __name__ == '__main__':
    ...
    # paths
    parser.add_argument('--model_path', type=str, default='model_weight/', help='检查点路径')
    parser.add_argument('--vocab_path', type=str, default='Data/IUdata_vocab_0threshold.pkl', help='词汇包路径')
    parser.add_argument('--image_dir', type=str, default='Data/NLMCXR_Frontal', help='图像路径')
    parser.add_argument('--json_dir', type=str, default='Data/IUdata_train.json', help='json文件路径')
    parser.add_argument('--log_path', default='./Results', type=str, help='日志路径.')

    # model parameters
    parser.add_argument('--resize_size', type=int, default=256, help='更改输入图像大小适应resnet')
    ...
    parser.add_argument('--embed_size', type=int, default=512, help='词向量长度')
    ...
    # training parameters
    parser.add_argument('--teach_rate', type=float, default=1.0, help='默认为1是因为论文里说了总是给encoder提供ground truth')
    parser.add_argument('--learning_rate', type=float, default=1e-4, help='学习率')
    parser.add_argument('--epochs', type=int, default=50, help='epochs in training')
    ...
    parser.add_argument('--eval_json_dir', type=str, default='Data/IUdata_test.json', help='测试json')
